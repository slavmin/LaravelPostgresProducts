        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <a href="{{ route('product.create') }}" class="rounded-md bg-green-200 text-green-800 px-3 py-2">Create
                        product</a>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="flex">
                                        @if($product->deleted_at)
                                            <form action="{{ route('product.restore', $product->id) }}" method="POST">
                                                ...
                                                @method('get')
                                                <button type="submit"
                                                        class="px-4 py-2 ml-2 bg-green-100 text-green-900 rounded-md hover:bg-green-200 focus:outline-none focus:bg-green-200">
                                                    Restore
                                                </button>
                                            </form>
                                            <form action="{{ route('product.delete-permanently', $product->id) }}"
                                                  method="POST">
                                                @csrf
                                                @method('delete')
                                                <button type="submit"
                                                        class="px-4 py-2 ml-2 bg-red-100 text-red-900 rounded-md hover:bg-red-200 focus:outline-none focus:bg-red-200">
                                                    Delete
                                                </button>
                                            </form>
                                        @else
                                            <a href="{{ route('product.edit', $product->id) }}"
                                               class="px-4 py-2 mr-1 bg-indigo-100 rounded-md text-indigo-900 hover:bg-indigo-200">Edit</a>
                                            <form action="{{ route('product.destroy', $product->id) }}" method="POST">
                                                @csrf
                                                @method('delete')
                                                <button type="submit"
                                                        class="px-4 py-2 ml-2 bg-yellow-100 text-yellow-900 rounded-md hover:bg-yellow-200 focus:outline-none focus:bg-yellow-200">
                                                    Delete
                                                </button>
                                            </form>
                                        @endif
